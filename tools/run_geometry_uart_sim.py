...
def exchange(cmd,payload=bytes(8),code=0,page_body=None,bad_crc=False):
    ...
    body=bytes((code,threshold&255,threshold>>8,1,15,0,0,0)) if page_body is None else page_body
...
for value in ((1,4),(1,2),(3,2),(4,1),(7,5)):
    zoom=value
...
for payload in (struct.pack('<HH',0,1)+bytes(4),struct.pack('<HH',1,0)+bytes(4),struct.pack('<HH',5,1)+bytes(4)):
