...
module median_modes_tb;
  ...
  video_processing #(.IMAGE_WIDTH(WIDTH),.ENABLE_GRAY(GRAY),.SOBEL_ADAPTIVE(ADAPTIVE)) raw_dut
    (.clk(clk),.rst_n(rst),.ENABLE_SOBEL(1'b0),.ENABLE_MEDIAN(1'b0),.BINARY_OUTPUT(INVERT==0),.SOBEL_THRESHOLD(12'(THRESHOLD)),
  ...
  video_processing #(.IMAGE_WIDTH(WIDTH),.ENABLE_GRAY(GRAY),.SOBEL_ADAPTIVE(ADAPTIVE)) med_dut
    (.clk(clk),.rst_n(rst),.ENABLE_SOBEL(1'b0),.ENABLE_MEDIAN(1'b1),.BINARY_OUTPUT(INVERT==0),.SOBEL_THRESHOLD(12'(THRESHOLD)),
  ...
  video_processing #(.IMAGE_WIDTH(WIDTH),.ENABLE_GRAY(GRAY),.SOBEL_ADAPTIVE(ADAPTIVE)) edge_dut
    (.clk(clk),.rst_n(rst),.ENABLE_SOBEL(1'b1),.ENABLE_MEDIAN(1'b0),.BINARY_OUTPUT(INVERT==0),.SOBEL_THRESHOLD(12'(THRESHOLD)),
  ...
  video_processing #(.IMAGE_WIDTH(WIDTH),.ENABLE_GRAY(GRAY),.SOBEL_ADAPTIVE(ADAPTIVE)) both_dut
    (.clk(clk),.rst_n(rst),.ENABLE_SOBEL(1'b1),.ENABLE_MEDIAN(1'b1),.BINARY_OUTPUT(INVERT==0),.SOBEL_THRESHOLD(12'(THRESHOLD)),
