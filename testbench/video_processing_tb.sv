    ...
    initial begin
        ...
        while (!$feof(fd)) begin
            ...
            if (count == 11) begin
                ...
                if(!ri) begin for(j=0;j<6;j=j+1) original_delay[j]=0; end
                else begin
                    for(j=5;j>0;j=j-1) original_delay[j]=original_delay[j-1];
                ...
                if ({ph,pv,pd,bypass_rgb} !== {eh,ev,ed,original_delay[5]}) $fatal(1,"bypass cycle %0d",n);
                if({ih,iv,id,inverted_rgb} !== {eh,ev,ed,(ed ? ~eb : 24'd0)}) $fatal(1,"inversion cycle %0d",n);
                if({dh,dv,dd,dynamic_rgb} !== {eh,ev,ed,(enable_live ? (ed ? (invert_live ? ~eb : eb) : 24'd0) : original_delay[5])})
